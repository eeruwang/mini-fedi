...
export const onRequestGet: PagesFunction<{ OAUTH_KV: KVNamespace }> = async ({ request, env }) => {
  ...
  const kvKey = `app:${iss}`
  let app = await env.OAUTH_KV.get(kvKey, { type: 'json' }) as any | null
  if (!app) {
    ...
    app = await reg.json()
    await env.OAUTH_KV.put(kvKey, JSON.stringify(app), { expirationTtl: 604800 })
  }

  const { verifier, challenge } = await pkcePair()
  const state = crypto.randomUUID()
  await env.OAUTH_KV.put(`state:${state}`, JSON.stringify({ iss, verifier }), { expirationTtl: 600 })
